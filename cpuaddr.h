...
{
	enum s9xwrap_t	wrap = (!CheckEmulation() || Registers.D.W != 0) ? WRAP_BANK : WRAP_PAGE;
	uint32	addr = S9xGetWord(DirectSlow(READ), wrap);
	if (a & READ)
...
{
	uint32	addr = S9xGetWord(DirectSlow(READ), Registers.D.W == 0 ? WRAP_PAGE : WRAP_BANK);
	if (a & READ)
		OpenBus = (uint8) (addr >> 8);
	addr |= ICPU.ShiftedDB;
...
{
	uint16	base = (uint16) DirectIndexedXSlow(READ);
	uint32	addr;

	// Undocumented emulation-mode quirk: when DL != 0, the high byte read wraps within the page.
	if (CheckEmulation() && Registers.DL)
	{
		uint8	low = S9xGetByte(base);
		uint16	base_high = (base & 0xff00) | ((base + 1) & 0x00ff);
		uint8	high = S9xGetByte(base_high);
		addr = low | (high << 8);
	}
	else
	{
		addr = S9xGetWord(base, (!CheckEmulation() || Registers.DL) ? WRAP_BANK : WRAP_PAGE);
	}
	if (a & READ)
		OpenBus = (uint8) (addr >> 8);
...
{
	uint16	base = (uint16) DirectIndexedXE1(READ);
	uint32	addr;
	...
	{
		addr = S9xGetWord(base, WRAP_PAGE);
	}
	if (a & READ)
		OpenBus = (uint8) (addr >> 8);
