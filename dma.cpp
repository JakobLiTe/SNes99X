...
uint8 S9xDoHDMA (uint8 byte)
{
	...
	// HDMA overhead: 18 master cycles per channel overhead
	// Based on anomie's SNES timing documentation
	// For each active channel: overhead = 18 master cycles (3 CPU cycles)
	// Plus 8 master cycles per byte transferred
	...
	// 18 master cycles = approximately 3 CPU cycles per channel
	ADD_CYCLES(numActiveChannels * 3);
